...
public class Analyze {
    ...
    public static List<Tuple> averageScoreBySubject(Stream<Pupil> stream) {
        return stream.map(pup -> pup.subjects()).
                ...
                entrySet().stream().
                map(t -> new Tuple(t.getKey(), t.getValue())).collect(Collectors.toList());

    }

    public static Tuple bestStudent(Stream<Pupil> stream) {
        return stream.map(s -> new Tuple(s.name(), s.subjects().stream().
                        mapToDouble(sc -> (double) sc.score()).sum())).
    ...
    public static Tuple bestSubject(Stream<Pupil> stream) {
        return stream.map(pup -> pup.subjects()).
                ...
                entrySet().stream().
                map(t -> new Tuple(t.getKey(), t.getValue())).max(Comparator.comparingDouble(Tuple::score))
                .orElse(null);
